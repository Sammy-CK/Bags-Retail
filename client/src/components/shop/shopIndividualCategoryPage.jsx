...
function ShopIndividualCategoryPage() {
    ...
    fetch(`https://bags-o7py.onrender.com/categories/${categoryID}`, {
      ...
      if (resp.ok) {
        return resp.json().then((data) => {
          ...
          setCategoryBags(
            data.bags.filter(
              (bag) => bag.secret_shop_key === +shopID && bag.sold == false
            )
          );
        });
      } else {
  ...
  let shownCategoryBags = categoryBags.map((categoryBag) => {
    return (
      <li className="bags-li" key={categoryBag.id}>
        <p className="bags-name">name: {categoryBag.name}</p>{" "}
        <img
          className="bags-img"
          src={categoryBag.image_url}
          width="220px"
          height="220px"
        />{" "}
        <button
  ...
  return (
    <div>
      ...
      <ul className="bags-ul">{shownCategoryBags}</ul>
    </div>
  );
}
